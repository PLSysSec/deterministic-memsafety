...
public:
  ...
  /// can be CLEAN, DIRTY, BLEMISHED, or UNKNOWN.
  DLIMAnalysis(Function &F, bool trustLLVMStructTypes, PointerKind inttoptr_kind) : F(F), trustLLVMStructTypes(trustLLVMStructTypes), inttoptr_kind(inttoptr_kind) {
...
PreservedAnalyses StaticDLIMPass::run(Function &F, FunctionAnalysisManager &FAM) {
  DLIMAnalysis analysis = DLIMAnalysis(F, true, CLEAN);
...
PreservedAnalyses ParanoidStaticDLIMPass::run(Function &F, FunctionAnalysisManager &FAM) {
  DLIMAnalysis analysis = DLIMAnalysis(F, false, DIRTY);
  DLIMAnalysis::StaticResults static_results = analysis.run();
  analysis.reportStaticResults(static_results);
