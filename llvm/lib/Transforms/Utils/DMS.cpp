...
public:
  ...
  struct Settings {
    ...
    DynamicPrintType dynamic_print_type;
...
private:
  ...
  void addDynamicResultsPrint(DynamicResults& dynamic_results) {
    ...
    if (settings.dynamic_print_type == STDOUT) {
      // call printf()
    ...
    } else if (settings.dynamic_print_type == TOFILE) {
      ...
      llvm_unreachable("unexpected dynamic_print_type\n");
    }
...
PreservedAnalyses DynamicDMSPass::run(Function &F, FunctionAnalysisManager &FAM) {
  ...
  settings.dynamic_print_type = DMSAnalysis::DynamicPrintType::TOFILE;
  settings.add_sw_spatial_checks = false;
...
PreservedAnalyses DynamicStdoutDMSPass::run(Function &F, FunctionAnalysisManager &FAM) {
  ...
  settings.do_pointer_encoding = true;
  settings.instrument_for_dynamic_counts = true;
  settings.dynamic_print_type = DMSAnalysis::DynamicPrintType::STDOUT;
  settings.add_sw_spatial_checks = false;
