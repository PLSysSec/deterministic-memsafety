...
public:
  struct Settings {
    ...
    /// the `PointerStatus` to use for pointers generated by `inttoptr`
    /// instructions, i.e., by casting an integer to a pointer. This can be any
    /// `PointerStatus` -- e.g., `Clean`, `Dirty`, `Unknown`, etc.
    PointerStatus inttoptr_status;
    ...
    /// where necessary.
    /// Currently this assumes that dereferencing `Blemished`(<=16) pointers
    /// does not need a SW check, but all other `Blemished` pointers need checks.
  ...
  DMSAnalysis(Function &F, FunctionAnalysisManager &FAM, Settings settings)
    : F(F), DL(F.getParent()->getDataLayout()),
      ...
      block_states(BlockStates(F, DL, settings.trust_llvm_struct_types, settings.inttoptr_status, added_insts, pointer_aliases)),
  ...
  StaticResults run() {
    ...
    while (res.changed) {
      ...
      iterationCount++;
      LLVM_DEBUG(dbgs() << "DMS: starting iteration " << iterationCount << " through function " << F.getName() << "\n");
...
private:
  ...
  /// Caller must only set this to `true` after the analysis has reached a
  /// fixpoint.
  /// Currently this assumes that dereferencing `Blemished`(<=16) pointers does
  /// not need a SW check, but all other `Blemished` pointers need checks.
  AnalyzeBlockResult analyze_block(
    ...
    #define COUNT_OP_AS_STATUS(category, status, ip, errcontext) { \
      const PointerStatus the_status = (status); /* in case (status) is an expensive-to-compute expression, compute it once here */ \
      if (the_status.is_clean()) { COUNT_OP_KIND(category, clean, ip) } \
      else if (the_status.is_blemished()) { \
        const PointerStatus::Blemished& blem = std::get<PointerStatus::Blemished>(the_status.data); \
        if (!blem.max_modification.has_value()) { COUNT_OP_KIND(category, blemishedconst, ip) } \
        else if (blem.max_modification->ule(16)) { COUNT_OP_KIND(category, blemished16, ip) } \
        else if (blem.max_modification->ule(32)) { COUNT_OP_KIND(category, blemished32, ip) } \
        else if (blem.max_modification->ule(64)) { COUNT_OP_KIND(category, blemished64, ip) } \
        else if (blem.max_modification->isNegative()) llvm_unreachable("max_modification should never be negative"); \
        else { COUNT_OP_KIND(category, blemishedconst, ip) } \
      } \
      else if (the_status.is_dirty()) { COUNT_OP_KIND(category, dirty, ip) } \
      else if (the_status.is_unknown()) { COUNT_OP_KIND(category, unknown, ip) } \
      else if (the_status.is_dynamic()) { \
        const PointerStatus::Dynamic& dyn = std::get<PointerStatus::Dynamic>(the_status.data); \
        COUNT_OP_DYN(category, dyn, ip) \
      } \
      else if (the_status.is_notdefinedyet()) { \
        errs() << errcontext; \
        llvm_unreachable("Expected a status, got `NotDefinedYet`"); \
      } \
      else { llvm_unreachable("PointerStatus case not handled"); } \
    }
      ...
      switch (inst.getOpcode()) {
        ...
        case Instruction::IntToPtr: {
          ...
          if (it != inttoptr_status_and_bounds_overrides.end()) {
            ...
            mark_as(ptr_statuses, &inttoptr, settings.inttoptr_status);
            if (settings.add_sw_spatial_checks) {
              bounds_infos.propagate_bounds(inttoptr, settings.inttoptr_status);
            }
          }
          break;
        }
        ...
        case Instruction::ExtractValue: {
          ...
          // the same PointerStatus as the original pointer which was inserted;
        ...
        case Instruction::ExtractElement: {
          ...
          if (vector_status.is_notdefinedyet()) {
        ...
        case Instruction::InsertElement: {
          ...
          if (existing_vector_status.is_notdefinedyet()) {
  ...
  bool maybeAddSpatialSWCheck(
    ...
    if (status.is_clean()) {
      ...
      const PointerStatus::Blemished& blem = std::get<PointerStatus::Blemished>(status.data);
      if (blem.under16()) {
        // no check required
        return false;
      } else {
      ...
      if (const BoundsInfo::Static* sinfo = std::get_if<BoundsInfo::Static>(&binfo->data)) {
        if (sinfo->fails(access_bytes)) {
          ...
          Value* not_clean = Builder.CreateICmpNE(
            dyn.dynamic_kind,
            Builder.getInt64(PointerStatus::Dynamic::Masks::clean),
            Twine(addr_name, "_not_clean")
          );
          Value* not_blem16 = Builder.CreateICmpNE(
            dyn.dynamic_kind,
            Builder.getInt64(PointerStatus::Dynamic::Masks::blemished16),
            Twine(addr_name, "_not_blem16")
          );
          ...
          // insert instructions to check the DynamicKind
          return false;
        }
      } else {
      ...
      llvm_unreachable("trying to bounds check on pointer with no status");
    } else {
  ...
  /// If dynamic instructions need to be inserted, use `Builder`.
  ///
  /// Specifically, our encoding uses bits 48-49 to indicate the dynamic status
  /// of the pointer, interpreted per the PointerStatus::Dynamic::DynamicKind
  /// enum.
  ...
  std::tuple<Instruction*, PointerStatus, User*> decode_ptr(Value* encoded_ptr, DMSIRBuilder& Builder) {
    ...
    Value* dynamic_kind = Builder.CreateAnd(encoded_ptr_as_int, PointerStatus::Dynamic::Masks::dynamic_kind_mask, Twine(ptr_name, "_dynamic_kind"));
    Value* decoded_ptr_as_int = Builder.CreateAnd(encoded_ptr_as_int, ~PointerStatus::Dynamic::Masks::dynamic_kind_mask);
  ...
  // based on the `PointerStatus::Dynamic`.
  void incrementGlobalCounterForDynKind(DynamicCounts& dyn_counts, const PointerStatus::Dynamic& dyn_status, Instruction* BeforeInst) {
    assert(dyn_status.dynamic_kind != NULL);
    DMSIRBuilder Builder(BeforeInst, DMSIRBuilder::BEFORE, &added_insts);
    Type* i64ty = Builder.getInt64Ty();
    Constant* null = Constant::getNullValue(dyn_counts.clean->getType());
    Value* GlobalCounter = Builder.CreateSelect(
      Builder.CreateICmpEQ(dyn_status.dynamic_kind, Builder.getInt64(PointerStatus::Dynamic::Masks::clean)),
      dyn_counts.clean,
      null);
    GlobalCounter = Builder.CreateSelect(
      Builder.CreateICmpEQ(dyn_status.dynamic_kind, Builder.getInt64(PointerStatus::Dynamic::Masks::blemished16)),
      dyn_counts.blemished16,
      GlobalCounter);
    GlobalCounter = Builder.CreateSelect(
      Builder.CreateICmpEQ(dyn_status.dynamic_kind, Builder.getInt64(PointerStatus::Dynamic::Masks::blemished_other)),
      dyn_counts.blemishedconst,  // conservative, as we don't know which BLEMISHED category to use
      GlobalCounter);
    GlobalCounter = Builder.CreateSelect(
      Builder.CreateICmpEQ(dyn_status.dynamic_kind, Builder.getInt64(PointerStatus::Dynamic::Masks::dirty)),
      dyn_counts.dirty,
      GlobalCounter);
    LoadInst* loaded = Builder.CreateLoad(i64ty, GlobalCounter);
    Value* incremented = Builder.CreateAdd(Builder.getInt64(1), loaded);
...
PreservedAnalyses StaticDMSPass::run(Function &F, FunctionAnalysisManager &FAM) {
  ...
  settings.inttoptr_status = PointerStatus::clean();
...
PreservedAnalyses ParanoidStaticDMSPass::run(Function &F, FunctionAnalysisManager &FAM) {
  ...
  settings.inttoptr_status = PointerStatus::dirty();
  settings.do_pointer_encoding = false;
  settings.instrument_for_dynamic_counts = false;
  settings.dynamic_print_type = DynamicResults::PrintType::STDOUT;
  settings.add_sw_spatial_checks = false;
