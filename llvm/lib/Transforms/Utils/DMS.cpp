...
static InductionPatternResult isOffsetAnInductionPattern(
  ...
  DEBUG_WITH_TYPE("DMS-loop-induction", dbgs() << "DMS:   Checking the following gep for induction:\n");
  DEBUG_WITH_TYPE("DMS-loop-induction", gep.dump());
  ...
  for (const Use& idx_as_use : gep.indices()) {
    ...
    if (ivr.is_induction_var) {
      DEBUG_WITH_TYPE("DMS-loop-induction", dbgs() << "DMS:     GEP single index is an induction var\n");
    } else {
      ...
      if (vpcr.valid) {
        ...
        if (ivr.is_induction_var) {
          DEBUG_WITH_TYPE("DMS-loop-induction", dbgs() << "DMS:     GEP single index is an induction var plus a constant " << vpcr.constant << "\n");
    ...
    if (!ivr.is_induction_var) {
      DEBUG_WITH_TYPE("DMS-loop-induction", dbgs() << "DMS:     not an induction pattern\n");
    ...
    if (success) {
      ...
      DEBUG_WITH_TYPE("DMS-loop-induction", dbgs() << "DMS:     induction pattern with initial " << ipr.initial_offset << " and induction " << ipr.induction_offset << "\n");
      return ipr;
    } else {
      DEBUG_WITH_TYPE("DMS-loop-induction", dbgs() << "DMS:     but failed the dereference-inside-loop check\n");
      return no_induction_pattern;
    }
  }
...
static InductionVarResult isInductionVar(const Value* val) {
  if (const PHINode* phi = dyn_cast<PHINode>(val)) {
    ...
    if (found_initial_val && found_induction_increment) {
      ...
      DEBUG_WITH_TYPE("DMS-loop-induction", dbgs() << "DMS:     Found an induction var, initial " << initial_val << " and induction " << induction_increment << "\n");
